...
def test_not_open_city_modal_window_first_time(main_page):
    page = MainPage(main_page)
    assert not page.open_modal_window_with_cities()
...
def test_add_item_to_basket(main_page):
    page = MainPage(main_page)
    assert page.add_item_to_basket()
